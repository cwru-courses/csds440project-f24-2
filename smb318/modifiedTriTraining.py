...
class modifiedTriTraining:
    ...
    def measure_error(c1, c2, data: np.ndarray, labels: np.ndarray) -> float:
        agreements = modifiedTriTraining.agreement_indices(c1, c2, data)
    ...
    # learners are the different classifiers (e.g. DecisionTreeClassifier, Naive Bayes) that we will use
    ...
    def measure_errors(self, data: np.ndarray, labels: np.ndarray) -> List[float]:
        ...
        for i in range(3):
            ...
            j = (i + 1) % 3
            k = (i + 2) % 3
            e[i] = modifiedTriTraining.measure_error(self.learners[j], self.learners[k], data, labels)
    ...
    def fit(self, L_data: np.ndarray, L_labels: np.ndarray, U_data: np.ndarray):
        ...
        # initializes learners. we can use the entire labeled data since we are using different base learners
        for i in range(3):
            self.learners[i].fit(L_data, L_labels)
        ...
        while any(update):
            for i in range(3):
                ...
                if e[i] < e_prime[i]:
                    ...
                    agreements = modifiedTriTraining.agreement_indices(self.learners[j], self.learners[k], U_data)
                    ...
                    if l_prime[i] < num_new_examples:
                        if e[i] * num_new_examples < e_prime[i] * l_prime[i]:
                            ...
                            L_new_data[i], L_new_labels[i] = modifiedTriTraining.subsample(L_new_data[i],
                                                                                           L_new_labels[i],
                                                                                           math.ceil(
                                                                                               e_prime[i] * l_prime[i] /
                                                                                               e[i] - 1))
